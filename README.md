...
When enabled with a threshold, the Pi footer shows `decompression 40% left/50%`, where `left` is the percentage-point distance to the configured decompression threshold (not total model context capacity). Usage is rounded up for a compact display, clamped to `0%` at or above the threshold, and shown as `decompression -- left/50%` when unavailable. Without a configured threshold it shows `decompression on:no-threshold`. Disabling decompression clears this footer item.
